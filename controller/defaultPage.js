...
// Using in nodeJs
exports.findFaqs = async (userData, locale, path_prefix = "", plan) => {
  let send_data = []
  let selectCondition = {}
  if (userData) {
    ...
    try {
        let selectQueryFaqs = "SELECT `faq`.`id`,`faq`.`title`, `faq`.`liked_faq`, `faq`.`disliked_faq`,`faq`.`content`,`faq`.`locale`,`faq`.`identify`,`faq`.`category_identify` FROM faq" +
...
exports.findSetting = async (userData, locale, plan) => {
  let returnData = {};
  let data = {};
  let templateSetting = {};
  if (userData) {
    ...
    await Setting.findOne({
        ...
        await Setting.update({
          view_faq_page : data.view_faq_page + 1
        },
          {
            where: {
              user_id: userData.dataValues.id

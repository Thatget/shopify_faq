...
app.get("/", async (req, res) => {
  ...
  if (!req.query.session) {
    ...
    const redirectUri = `${forwardingAddress}/shopify/callback`;
    const permissionUrl = `https://${req.query.shop}/admin/oauth/authorize?client_id=${apiKey}&scope=${scopes}&redirect_uri=${redirectUri}`;
...
app.get("/api/select/plan", async (req, res) => {
  ...
  if (req.query.price == "0" && req.query.plan != freePlan) {
    ...
    if (
      session.body.data.appSubscriptionCancel.appSubscription.status ===
      "CANCELLED"
    ) {
      await Plan.update(
        {
...
app.get("/faq-proxy", async (req, res) => {
  ...
  const faqData = await defaultPage.findFaqs(
    shop,
    locale,
    path_prefix,
    plan,
  ...
  return res.set("Content-Type", "application/liquid").render("loadMoreFaq", {
    faqs: faqData,
    setting: setting_,
  ...
  if (shop) {
    try {
      ...
      if (userData.id != 3520) {
        ...
        if (resp.body.data.currentAppInstallation.activeSubscriptions) {
          currentPlan =
            resp.body.data.currentAppInstallation.activeSubscriptions;
        }
      }
      await Plan.findOne({
        attributes: ["plan", "id"],
        ...
        .then(async (data) => {
          if (currentPlan.length > 0) {
            ...
            if (
              data.dataValues.plan == proPlan ||
              data.dataValues.plan == ultimatePlan
            ) {
              await Plan.update(
                {
                  plan: freePlan,
      ...
      const faqs = await defaultPage.findFaqs(
        shop,
        locale,
        path_prefix,
        plan,
